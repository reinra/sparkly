...
interface TypedRequest<T extends EndpointDefinition> extends Request {
  body: InferBody<T>;
}
...
type TypedHandler<T extends EndpointDefinition> = (req: TypedRequest<T>, res: TypedResponse<T>) => Promise<void> | void;
...
export function createTypedHandler<T extends EndpointDefinition>(endpoint: T, handler: TypedHandler<T>) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await handler(req as TypedRequest<T>, res as TypedResponse<T>);
    } catch (error) {
